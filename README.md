...
- **官方侧边栏树零改动**：每台服务器自动成为官方工作区树中的一个分组（虚拟 workspace，`workspace.list` 合并 + 合成 `host/workspace-changed` 帧实时同步），会话点击即打开；
...
**凭据**：不读取、不存储、不中继任何 `credentials`/`settings` 域（官方本就固定在环回）。

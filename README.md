...
**不适合谁**：只有一台机器、也没用过这几种工具的人，这插件对他没有价值。
...
- 勾选**自动**：文件写入会触发增量跟进，新消息大约一两秒内出现在会话里；另有 60 秒周期扫描兜底，
  覆盖文件监听失效的平台。不勾选就只在你点刷新时更新；
- 正在活跃写入的会话带**运行中**状态点，停止写入后自动熄灭；
...
客户的生产机、不允许装东西的跳板机，那是 `dsh-remote` 的场子，本插件无能为力。而想要“关机之后任务继续跑”，则必须让远端自己有一个 `dsh`。
